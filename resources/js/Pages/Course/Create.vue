<template>
    <Dialog
        v-model:visible="courseStore.createModal"
    ...
    >
        ...
        <!-- Title -->
            ...
            <div class="flex flex-col gap-2">
             ...
             <div class="flex flex-col gap-2">
                <label>Price</label>
                <InputNumber v-model="courseStore.form.price" placeholder="Price" />
                <small v-if="courseStore.errors.price" class="text-red-500">{{ courseStore.errors.price }}</small>
            </div>

            <!-- Actions -->
            <div class="flex justify-end gap-2">
                <Button label="Cancel" severity="secondary" @click="courseStore.closeCreate" />
                <SingleClickButton label="Create" lockLabel="Creating..." @click="submit" />
            </div>
        </div>
    </Dialog>
</template>
...
const submit = async () => {
  try {
    const response = await courseStore.createCourse(); // await the async function
