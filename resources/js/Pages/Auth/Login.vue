...
const submit = async () => {
    try {
        ...
        if (response && response.status >= 200 && response.status < 300) {
            toastSuccess(toast, "Login", response.data?.message || "Logged in");
            router.get(response.data?.redirect || "/dashboard");
        }
    } catch (error) {
        const status = error.response?.status;
        const fieldErrors = error.response?.data?.errors;
        const firstFieldError = fieldErrors ? Object.values(fieldErrors)[0]?.[0] : null;
        let message = firstFieldError || error.response?.data?.message || error.message;

        if (status === 401 || status === 403) {
            message = message || "Invalid email or password.";
        }

        toastError(toast, "Login", message || "Login failed");
    }
};
</script>
