<template>
    <Dialog
        v-model:visible="categoryStore.createModal"
        header="Create Course Category"
        modal
        :closable="false"
        :style="{ width: '30rem' }"
    >
        <div class="flex flex-col gap-4">
            ...
            <div class="flex justify-end gap-2">
                <Button label="Cancel" severity="secondary" @click="categoryStore.closeCreate" />
...
const submit = async () => {
  try {
    await categoryStore.createCourseCategory(toast);
  } catch (err) {
